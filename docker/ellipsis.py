...
# Function to list timestamps in a folder
...
def delete_timestamp(path_id_layer, timestamp_id):
    ...
    delete_url = f"{base_url}/path/{path_id_layer}/vector/timestamp/{timestamp_id}/trashed"
    params = {
        "pathId": path_id_layer,
    ...
    response = requests.put(delete_url, data=params, headers=headers_upload,)
...
def create_timestamp(path_id):
    ...
    response_timestamp = requests.post(add_timestamp_url, headers=headers_timestamp, data=json.dumps(payload_timestamp))
...
def upload_file(path_id, timestamp_id, file_name):
    ...
    response_upload = requests.post(upload_url, files=files, data=params, headers=headers_upload)
    return response_upload




def refresh_timestamps(file_name,
                       path_id = "b11909f0-3c51-4ab2-a57e-ffaa60335770"
                       ):
        ...
        for item in timestamps:
            ...
            for timestamp in item['vector']['timestamps']:
                ...
                if delete_response.status_code == 200:
                    ...
                    ic(f"Failed to delete timestamp ID: {timestamp_id}. Status code: {delete_response.status_code}")
        ...
        if create_response.status_code == 200:
            ...
            upload_response = upload_file("49821b18-5a0f-4b5f-871d-6442d1c72d86", timestamp_id, file_name)
